...
router.get("/content/stores", async (req, res) => {
  try {
    await seedDefaultData();
    const rows = await db
      .select()
      ...
      .orderBy(asc(stores.order), asc(stores.id));
    res.json(rows);
  } catch (err) {
    req.log.error({ err }, "content/stores error");
    res.status(500).json({ error: "Erro interno" });
  }
});

router.get("/content/apps", async (req, res) => {
  try {
    await seedDefaultData();
    const rows = await db
      .select()
      ...
      .orderBy(asc(apps.order), asc(apps.id));
    res.json(rows);
  } catch (err) {
    req.log.error({ err }, "content/apps error");
    res.status(500).json({ error: "Erro interno" });
  }
});
